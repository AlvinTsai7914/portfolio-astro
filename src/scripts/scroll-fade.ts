...
//   頁面載入直接播放：<div data-fade-up data-fade-immediate>
//
// 可選 attribute：
//   data-fade-delay="0.2"     延遲（秒），預設 0
//   data-fade-duration="0.8"  時長（秒），預設 0.5
//   data-fade-stagger="0.2"   群組間隔（秒），預設 0.15
//   data-fade-y="60"          Y 位移（px 或 %），預設 "100%"
//   data-fade-trigger=".selector"  自訂觸發錨點（CSS 選擇器），預設為元素自身
...
function initScrollFade() {
  ...
  singles.forEach((el, index) => {
    ...
    const immediate = el.dataset.fadeImmediate !== undefined;
  ...
  groups.forEach((elements, name) => {
    ...
    const immediate = first.dataset.fadeImmediate !== undefined;

    gsap.set(elements, { opacity: 0, y });

    if (immediate) {
      const tween = gsap.to(elements, {
        opacity: 1, y: 0, duration, delay, stagger, ease: EASE,
      });
      activeTweens.push(tween);
    } else {
      ScrollTrigger.create({
